...
class ModalUpdatePage extends Component {
state = {
  modal16: false,
  sendHandle : {
...
post_handle(){
  ...
  API.processTicket.updateTicket(this.state.sendHandle,authHeader()).then(res=>{
    if(res.status == 1){
      this.setState({
        success:true,
        loading:false
...
render() {
  return (
    <Fragment>
        ...
        <MDBIcon icon="comment" className="mr-2" />
            Update
        </MDBBtn>
      <MDBContainer>
        <MDBModal isOpen={this.state.modal16} toggle={this.toggle(16)} size="lg" centered>
         {
          ...
          <MDBModalHeader toggle={this.toggle(16)}>Update Ticket Info</MDBModalHeader>
          <MDBModalBody>
